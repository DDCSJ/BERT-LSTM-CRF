...
def bert_train(**kwargs):
    ...
    vocab = load_vocab(config.bert_vocab) # {token: index}
    ...
    model = BERT_LSTM_CRF(config.bert_path, tagset_size, config.ernie_embedding, config.rnn_hidden, config.rnn_layer, dropout_ratio=config.dropout_ratio, dropout1=config.dropout1, use_cuda=config.use_cuda)
